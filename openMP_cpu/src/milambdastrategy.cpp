...
MiLambdaStrategy::MiLambdaStrategy(const int mi, const int lambda, const int iterations) : Method() {
	this->mi=mi;
	this->lambda=lambda;
...
Point MiLambdaStrategy::operator () (const std::vector<Point*>& initialPoints) const{
    std::cout<<"MiLambdaStrategy operator().\n";
...
//        basePopulation.erase(basePopulation.begin());
        ...
        std::cout<<"populationEvals:\t"<<basePopulation[0]->getEval()<<"\t-\t"<<basePopulation[mi-1]->getEval()<<"\n";		
